CLASS zcl_pdjs_testdata DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
    INTERFACES if_oo_adt_classrun.

    CONSTANTS:
      c_error                     TYPE zdjs_items-price VALUE '37707',
  ...
  PRIVATE SECTION.
    DATA:
      mt_partner         TYPE STANDARD TABLE OF zdjs_partner,
      mt_material        TYPE STANDARD TABLE OF zdjs_material,
      mt_discount        TYPE STANDARD TABLE OF zdjs_discount,
      mt_head            TYPE STANDARD TABLE OF zdjs_invoice,
      mt_position        TYPE STANDARD TABLE OF zdjs_items,

      mo_random_partner  TYPE REF TO zcl_pdjs_random,
      mo_random_date     TYPE REF TO zcl_pdjs_random,
      mo_random_position TYPE REF TO zcl_pdjs_random,
      mo_random_material TYPE REF TO zcl_pdjs_random,
      mo_random_quantity TYPE REF TO zcl_pdjs_random.
    ...
    METHODS:
      ...
      create_head
        RETURNING
          VALUE(rs_result) TYPE zdjs_invoice,

      create_positions
        IMPORTING
          is_head TYPE zdjs_invoice.
ENDCLASS.


CLASS zcl_pdjs_testdata IMPLEMENTATION.
  ...
  METHOD create_partner.
    mt_partner = VALUE #( ( partner = '1000000000' name = 'SAP' street = 'Demo Street 15' city = 'Walldorf' country = 'DE' payment_currency = 'EUR' )
    ...
    DELETE FROM zdjs_partner.
    INSERT zdjs_partner FROM TABLE @mt_partner.
  ENDMETHOD.

  METHOD create_material.
    mt_material = VALUE #(
                          ( material = 'F0001' name = 'Peanuts' description = 'Roasted Peanuts from US' stock = '900' stock_unit = 'ST' price_per_unit = '2.50' currency = 'USD' )
                          ( material = 'F0002' name = 'Rice' description = 'Big bag rice from china' stock = '120' stock_unit = 'BAG' price_per_unit = '12.00' currency = 'USD' )
                          ( material = 'F0003' name = 'Eggs' description = 'Eggs from happy german chickens' stock = '550' stock_unit = 'PAK' price_per_unit = '3.15' currency = 'EUR' )
                          ( material = 'H0001' name = 'USB Stick 128 GB' description = 'USB Stick with security features' stock = '30' stock_unit = 'ST' price_per_unit = '49.99' currency = 'EUR' )
                          ( material = 'H0002' name = 'OLED Display 34"' description = 'Big and wide display with HDMI and dsiplay port' stock = '18' stock_unit = 'ST' price_per_unit = '440.00' currency = 'USD' )
                          ( material = 'R0001' name = 'Gas' description = 'Gas from sibiria' stock = '50000' stock_unit = 'MMQ' price_per_unit = '1560.00' currency = 'RUB' ) ).

    DELETE FROM zdjs_material.
    INSERT zdjs_material FROM TABLE @mt_material.
  ENDMETHOD.
  ...
  METHOD create_discount.
    ...
    DELETE FROM zdjs_discount.
    INSERT zdjs_discount FROM TABLE @mt_discount.
  ENDMETHOD.
  ...
  METHOD create_invoice.
    ...
    DELETE FROM zdjs_invoice.
    INSERT zdjs_invoice FROM TABLE @mt_head.
    DELETE FROM zdjs_items.
    INSERT zdjs_items FROM TABLE @mt_position.
  ENDMETHOD.
  ...
  METHOD create_positions.
    ...
    DO mo_random_position->rand( ) TIMES.
      ...
      DATA(ls_position) = VALUE zdjs_items(
      ...
      TRY.
          SELECT SINGLE FROM zdjs_discount
            FIELDS
              currency_conversion(
